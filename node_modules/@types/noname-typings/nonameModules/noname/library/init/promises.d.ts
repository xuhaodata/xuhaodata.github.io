export class LibInitPromises {
     ...
     * @returns {Promise<URL>}
     */
    parseResourceAddress(link: string | URL, defaultHandle?: ((item: string) => string) | null | undefined, forceLoadAsDataUrl?: boolean | undefined): Promise<URL>;
    /**
     * @async
     * @param {string | URL} link - 需要解析的路径
     * @param {((item: string) => string) | null} [defaultHandle] - 在给定路径不符合可用情况（或基于无名杀相关默认情况）时，处理路径的函数，返回的路径应是相对于根目录的相对路径，默认为`null`，当且仅当无法解析成`URL`时会调用该回调
     ...
     */
    parseResourceAddressExt(link: string | URL, defaultHandle?: ((item: string) => string) | null | undefined): Promise<[origin: URL, data: URL]>;
}
