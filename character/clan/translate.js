const translates = {
	...
	clan_wuyi_prefix: "族",
	clangaojin: "高劲",
	clangaojin_info: "锁定技，游戏开始时你计算与其他角色的距离-X，当你计算与其他角色的距离发生变化后，你摸一张牌（X为你的手牌上限）。",
	clangaojin_append: `<span style="font-family: yuanli"><li><span style="text-decoration: line-through;">补丁×</span>注意事项：<br>真正意义上的距离变化后时机在无名杀现有框架无法实现，请不要反馈在某些情况距离变化后此技能不发动的问题。</span>`,
	clanpoxi: "破袭",
	...
	clanqieyi: "切议",
	clanqieyi_info: "出牌阶段开始时，你可观看牌堆顶两张牌。若如此做，本回合你使用每个花色的第一张牌结算完成后，展示牌堆顶的一张牌，若这两张牌：花色或类别相同，你获得展示牌；均不同，你将一张牌置于牌堆顶。",
	clanjianzhi: "谏直",
	clanjianzhi_info: "锁定技，结束阶段，你进行至多X次判定（X为你本回合〖切议〗的发动次数）。你将与你本回合使用过的花色相同的判定牌分配给任意角色；若判定牌中有你本回合未使用过的花色，你受到一点无来源雷电伤害。",
	clanquhuo: "去惑",
	clanquhuo_info: "宗族技，你每回合不因使用、打出或弃置而首次失去手牌中的【酒】或装备牌后，你可以令一名同族角色回复1点体力。",
	clan_yangxiu: "族杨修",
	clan_yangxiu_prefix: "族",
	clanjiewu: "捷悟",
	clanjiewu_info: "出牌阶段开始时，你可以令一名角色的手牌此阶段始终对你可见。然后你此阶段使用牌指定目标后，你可以展示「捷悟」角色一张手牌，若：两张牌花色相同，你摸一张牌，若此牌本回合以此法展示过的次数大于1，你将你与其之中手牌较多的角色一张牌置于牌堆顶（若该角色为你，则改为将你的一张牌置于牌堆顶）。",
	clangaoshi: "高视",
	clangaoshi_info: "结束阶段，你可以亮出牌堆顶X张牌（X为本回合你发动〖捷悟〗的次数），然后可以使用其中本回合出牌阶段你未使用过的牌名的牌，若你因此使用了所有亮出牌，你摸两张牌。",
	clan_xunshuang: "族荀爽",
	clan_xunshuang_prefix: "族",
	clanyangji: "佯疾",
	clanyangji_info: "准备阶段，或你体力值变化过的回合结束时，你可以展示所有手牌，然后依次使用其中的黑色牌，直到你以此法无法使用或造成伤害。然后若以此法使用的最后一张为黑桃牌，你将之作为【乐不思蜀】置于当前回合角色的判定区。",
	clandandao: "耽道",
