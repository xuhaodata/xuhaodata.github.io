...
export class Card extends HTMLDivElement {
	...
	addGaintag(gaintag) {
		//目前无名杀支持永久标记，只要加上前缀“etern_”（即eternal，永恒的）均视为永久标记
		//然后就是永久标记的翻译问题，前缀是etern_可以选择只看后面的子字符串的对应翻译
		...
		for (var gi = 0; gi < this.gaintag.length; gi++) {
			...
			if (!translate && tag.indexOf("etern_") == 0) translate = get.translation(tag.slice(6));
